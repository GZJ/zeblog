...
(defun zeblog-post-rename()
  ...
  (let ((new (read-string "new name:"))
	...
	(rename-file (zeblog-posts-publish-path old ) (zeblog-posts-publish-path new))
      )
    (if (file-exists-p post-files-published)
	(rename-file (zeblog-posts-files-publish-path old ) (zeblog-posts-files-publish-path new))
      )
    (with-current-buffer
...
(defun zeblog-publish-clean()
  ...
  (if (eq diff nil)
      ()
    (dolist (file diff)
      (let ((f (zeblog-posts-publish-path (file-name-sans-extension file))))
